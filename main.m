function main(NumOfNormalSamples,NumOfCancerSamples,NumOfChromosoms)
%  each chromosome has 2 column, the first column is Location and the second column 
% is CpG value. Data extention is txt.
clc;
NumOfNormalSamples=str2num(NumOfNormalSamples);
NumOfCancerSamples=str2num(NumOfCancerSamples);
NumOfChromosoms=str2num(NumOfChromosoms);
...
for chromosome=1:NumOfChromosoms
    ...
    for sampleNumber=1:NumOfCancerSamples
        A = importdata(strcat('Cancer/Sample',num2str(sampleNumber),'/Chromosome',num2str(chromosome),'/',num2str(chromosome),'.txt'));
