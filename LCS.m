...
for i = 2:n+1
    for j = 2:m+1
        %if (X(i-1) == Y(j-1))
        %if (X(i-1)-.25 <= Y(j-1) && Y(j-1)<=X(i-1)+.25)
        %if (Y(j-1)<=X(i-1)+.25) % find Hyper regions
        %if (Y(j-1)>=X(i-1)+.25) % find Hypo regions
        if (abs(Y(j-1)-X(i-1))>=.25) % find Both
            L(i,j) = L(i-1,j-1) + 1;
            b(i,j) = 3;%%%Up and left
        else
            L(i,j) = L(i-1,j-1);
